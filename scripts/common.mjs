...
export async function files(dir, prefix = "") {
  ...
  for (const entry of await readdir(path.join(dir, prefix), {
    ...
    if (entry.isSymbolicLink())
      throw new Error(`Unexpected symlink: ${relative}`);
...
export function executableTokens(source) {
  return Array.from(
    tokenizer(source, { ecmaVersion: "latest", sourceType: "script" }),
    (token) => [token.type.label, source.slice(token.start, token.end)],
  );
}
