...
import { baseline, extension, root, jsFiles, sha256 } from "./common.mjs";
...
for (const [variant, directory] of [
  ...
  const profile = await mkdtemp(
    path.join(os.tmpdir(), "docs-offline-investigation-"),
  );
  ...
  try {
    ...
    await context.route("https://docs.google.com/**", (route) =>
      route.fulfill({
        ...
        body: "<!doctype html><title>Synthetic offline investigation fixture</title>",
      }),
    );
    const page = await context.newPage();
    await page.goto("https://docs.google.com/offline-investigation-fixture", {
      waitUntil: "domcontentloaded",
    });
    await page.addScriptTag({
      url: `chrome-extension://${id}/page_embed_script.js`,
    });
        ...
        async ({ id, message }) => {
          return Promise.race([
            new Promise((resolve, reject) =>
              chrome.runtime.sendMessage(id, message, (reply) =>
            ...
            new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error("External message timeout")),
                12000,
              ),
            ),
          ]);
    ...
    const optIn = await send([
      ...
      null,
      null,
      ["offline-investigation-synthetic-user", true],
    ]);
    ...
    const nativeState = await worker.evaluate(async () => ({
      ...
      ).map((c) => new URL(c.documentUrl).pathname),
    }));
    ...
    assert.deepEqual(exited, {
      storage: { offlineOptedIn: false },
      heartbeat: null,
      offscreenCount: 0,
    });
    const testedJavaScript = Object.fromEntries(
      await Promise.all(
    ...
    results.push({
      ...
      nativeState: {
        ...nativeState,
        heartbeat: {
          name: nativeState.heartbeat.name,
          periodInMinutes: nativeState.heartbeat.periodInMinutes,
        },
      },
      exited,
      profile,
    ...
    console.log(
      `PASS Chromium ${variant}: native worker, external messages, probe, offscreen, heartbeat, opt-out.`,
    );
  } finally {
    await context.close();
  }
}
const comparable = ({ variant, profile, testedJavaScript, ...rest }) => rest;
assert.deepEqual(comparable(results[0]), comparable(results[1]));
await mkdir(path.join(root, "research/validation"), { recursive: true });
await writeFile(
  path.join(root, "research/validation/browser-smoke.json"),
  JSON.stringify(
    {
      completedAt: new Date().toISOString(),
      scope:
        "Native Chrome extension API comparison in separate synthetic profiles. Google traffic blocked; no Google backend or real document sync tested.",
      results,
    },
    null,
    2,
