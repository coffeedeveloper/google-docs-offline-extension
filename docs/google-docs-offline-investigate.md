...
| iframe 握手超时 | 14 秒后进入失败路径，让日志排空与额外 14 秒延迟竞速，任一完成即可继续关闭；不是等待两者全部完成 |
...
2026-09-23 补充：`src/vendor/` 也已进行语义化还原，包括两个目标的全部全局绑定、核心局部变量、职责章节与中文状态机说明，保留兼容性敏感的短属性。当前通过绑定归一化 AST、可重复生成和新增 20 项运行库/还原器测试验证；不再要求 vendor token 不变。详细原理、字段表和维护方式见 [运行库阅读指南](../src/vendor/README.md)。本轮顺便纠正了早期对隐藏页 `Eg` 的解释：它是 race，不是 all；原运行行为没有改变。
...
- **来源与构建完整性**：原始样本哈希不变；85 个非 JS 资源字节相同；vendor 提取区间、词法绑定归一化 AST 和逐绑定命名映射可追溯；三个入口可确定性构建且 source map 与源码匹配。重构业务不宣称与原版 AST/token 一致。
...
| 检查 | 最终结果 |
| --- | --- |
| 原始样本与运行库来源 | 88 个原始资源 SHA-256 不变；vendor 原始区间、绑定归一化 AST 及确定性语义还原校验通过 |
